...
- Symptom: after a relaunch, the next tab move snapped instantly instead of tweening. Three mechanisms combine to produce it:
  1. `window.frame_monotonic` is set to `target_frame` when an animation is *created*, and `AnimationManager::animate_layout` skips any later layout whose target `same_as` it (`src/actor/reactor/animation.rs:174`).
  2. `WindowTxStore::insert` records the same target, and `animate_layout` skips again when `TransactionManager::get_target_frame` matches (`src/actor/reactor/transaction_manager.rs:70`, `src/model/tx_store.rs:44`).
  3. The app actor coalesces `Request::AnimationFrame` into `pending_frames: HashMap<WindowId, PendingFrame>` and writes at most one frame per window per drained batch (`State::handle_request_batch` / `flush_all_frames` in `src/actor/app.rs:555-612`).
- Landed in this ship: two bounded guards. (a) App actor: `Request::AnimationFrame` flushes a pending frame whose `txid` differs, so a mid-flight replacement cannot have its last write dropped — frames within one animation (same txid) still coalesce, preserving backpressure. (b) Reactor: one-frame bool guard `Reactor::suppress_next_redundant_animation_check` (src/actor/reactor.rs, src/actor/reactor/managers.rs, src/actor/reactor/animation.rs) armed at startup (relaunch) and consumed by the next `apply_layout` batch, bypassing `same_as`/`get_target_frame` once so the post-relaunch tween cannot be latched instant. See inline `ponytail:` / `rift-ship-01:` comments for ceiling.
