...
- Verification for *this* ship (what the guard can actually satisfy): trigger a layout change that replaces an in-flight animation for the same window, and confirm the superseded transaction's last frame is written instead of being dropped — the app actor should emit two writes with different `txid`s rather than one.
