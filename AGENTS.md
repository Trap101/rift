...
- Fixed in this ship: only (3), and only at transaction boundaries. `Request::AnimationFrame` now flushes a pending frame whose `txid` differs from the incoming one, so an animation replaced mid-flight cannot have its last write silently dropped by its successor's first frame. Frames within a single animation still coalesce, preserving the per-batch write backpressure the `HashMap` was built for.
- Still open: (1) and (2) are reactor-owned and untouched. They read state written at animation-creation time and get no feedback from intermediate `AnimationFrame` requests (`BeginWindowAnimation` suppresses the `WindowMoved`/`WindowResized` observers for the animation's duration), so the app-actor guard above cannot by itself unlatch them. Closing the latch means changing the `frame_monotonic` / tx-target skip predicates themselves.
...
- Manual trace to confirm: after relaunch, a tab drag that previously latched instant should deliver ~30 `AnimationFrame` ticks at ~10 ms rather than a single `anim.none` snap.
